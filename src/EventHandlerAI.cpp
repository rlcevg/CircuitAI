...
int CEventHandlerAI::HandleEvent(int topic, const void* data)
{
	switch (topic) {
		case EVENT_INIT: {
			...
			circuit->Init(evt->skirmishAIId, evt->callback);
			callback->GetLog()->DoLog("Let it be inited!!!");
			break;
		}
		case EVENT_RELEASE: {
			struct SReleaseEvent* evt = (struct SReleaseEvent*)data;
			circuit->Release(evt->reason);
			callback->GetLog()->DoLog("Let it be released!!!");
			break;
		}
		case EVENT_UPDATE: {
			struct SUpdateEvent* evt = (struct SUpdateEvent*)data;
			circuit->Update(evt->frame);
			break;
		}
		case EVENT_MESSAGE: {
		...
		case EVENT_LUA_MESSAGE: {
			...
			callback->GetLog()->DoLog(evt->inData);
