...
namespace circuit {
using WalkPosition = int2;
#define bwem_assert assert
class CTerrainData;
struct Area {
	using id = int;
	Area(CTerrainData* map, id i, WalkPosition p, int miniTiles) {}
};
//namespace BWEM {
//
//class Neutral;
...
namespace detail { using Graph = CTerrainData; }
...
//	const std::pair<const Area *, const Area *> & GetAreas() const	{ return m_Areas; }
	...
	const WalkPosition &					Center() const			{ return Pos(middle); }
	...
	const WalkPosition &					Pos(node n) const		{ bwem_assert(n < node_count); return m_nodes[n]; }
	...
	const WalkPosition &					PosInArea(node n, const Area::id pArea) const;
	...
	const std::deque<WalkPosition> &		Geometry() const		{ return m_Geometry; }
...
//	Map *									GetMap() const;
	CTerrainData *							GetMap() const { return m_pGraph; }
...
//	Details: The functions below are used by the BWEM's internals
	...
	typedef int								index;
//											ChokePoint(detail::Graph * pGraph, index idx, const Area * area1, const Area * area2, const std::deque<WalkPosition> & Geometry, Neutral * pBlockingNeutral = nullptr);
											ChokePoint(detail::Graph * pGraph, index idx, const Area::id area1, const Area::id area2, const std::deque<WalkPosition> & Geometry);
...
//	const std::pair<const Area *, const Area *>			m_Areas;
	const std::pair<const Area::id, const Area::id>		m_Areas;
	WalkPosition										m_nodes[node_count];
	std::pair<WalkPosition, WalkPosition>				m_nodesInArea[node_count];
	const std::deque<WalkPosition>						m_Geometry;
	bool												m_blocked;
