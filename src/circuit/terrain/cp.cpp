...
ChokePoint::ChokePoint(detail::Graph * pGraph, index idx, const Area::id area1, const Area::id area2, const deque<WalkPosition> & Geometry)
: m_pGraph(pGraph), m_index(idx), m_Areas(area1, area2), m_Geometry(Geometry),
	/*m_pBlockingNeutral(pBlockingNeutral), */m_blocked(/*pBlockingNeutral != nullptr*/false), m_pseudo(/*pBlockingNeutral != nullptr*/false)
{
...
//	if (m_pBlockingNeutral) m_pBlockingNeutral = GetMap()->GetTile(m_pBlockingNeutral->TopLeft()).GetNeutral();
	...
	for (int n = 0 ; n < node_count ; ++n)
		for (const Area::id pArea : {area1, area2})
		{
...
//const WalkPosition & ChokePoint::PosInArea(node n, const Area * pArea) const
const WalkPosition & ChokePoint::PosInArea(node n, const Area::id pArea) const
{
