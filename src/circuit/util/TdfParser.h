...
public:
	...
	TdfParser(circuit::CCircuitAI* ai) : circuit(ai) {};
	TdfParser(circuit::CCircuitAI* ai, const char* buffer, size_t size);
	virtual ~TdfParser();
	...
	template<typename T>
	void ParseArray(std::string const& value, T *array, size_t length) const;
...
private:
	circuit::CCircuitAI* circuit;  // logger
	TdfSection root_section;
	std::string filename;

	std::vector<std::string> GetLocationVector(std::string const& location) const;

	bool parse_buffer(char const* buf, size_t size);

public:
	springai::AIFloat3 GetFloat3(springai::AIFloat3 def, std::string const& location) const;
};
...
template<typename T>
void TdfParser::ParseArray(std::string const& value, T *array, size_t length) const
{
	std::stringstream stream;
	stream << value;
