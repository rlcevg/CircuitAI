...
public:
	CMoveAction(CCircuitUnit* owner, int squareSize, float speed = MAX_UNIT_SPEED);
	CMoveAction(CCircuitUnit* owner, const std::shared_ptr<F3Vec>& pPath, int squareSize, float speed = MAX_UNIT_SPEED);
	virtual ~CMoveAction();
